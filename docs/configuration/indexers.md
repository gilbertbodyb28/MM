...
* `max_results`\
  Maximum number of results retained from each indexer search. The default and
  upper bound are `1000`. Lower this value if an indexer returns unusually large
  feeds.
...
* `max_results`\
  Maximum number of results retained from each configured Jackett indexer. The
  default and upper bound are `1000`.
