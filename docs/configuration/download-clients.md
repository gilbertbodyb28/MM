...
* `category_name`\
  Category created and maintained by MediaManager. Default is `MediaManager`.
* `category_save_path`\
  Absolute path as seen by the qBittorrent container. It must point to the same
  mounted directory as MediaManager's `misc.torrent_directory`.
