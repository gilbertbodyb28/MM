...
- automatic Prowlarr/Jackett searches with qBittorrent, Transmission, or SABnzbd
- a Seerr-inspired Discover catalog with advanced filters
- private Ollama recommendations based on Plex or Tautulli history
...
# Edit ./config/config.toml, then build this enhanced source tree.
docker compose up -d --build
```

Optional local integrations are grouped into Compose profiles:

```sh
# Prowlarr + qBittorrent
docker compose --profile downloads up -d --build

# Or Jackett + qBittorrent
docker compose --profile jackett up -d --build

# Ollama + Tautulli
docker compose --profile ai up -d --build

# Bundled Discover-capable metadata relay (requires TMDB_API_KEY)
TMDB_API_KEY=replace-me docker compose --profile discover-relay up -d --build
```

See [Automatic Downloads](docs/advanced-features/automatic-downloads.md),
[Discover](docs/advanced-features/discover.md), and
[Personal AI Recommendations](docs/advanced-features/personal-recommendations.md)
