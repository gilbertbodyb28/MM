...
def _download_directory_name(title: str, torrent_hash: str) -> str:
    """Return a safe, stable directory name for a downloaded release."""
    directory_name = sanitize_filename(title, replacement_text="_").strip(" .")
    if not directory_name or directory_name in {".", ".."}:
        return torrent_hash
    return directory_name


def _download_path(category_save_path: str, directory_name: str) -> str:
    """Join qBittorrent paths without assuming the server's operating system."""
    windows_path = PureWindowsPath(category_save_path)
    if windows_path.drive or category_save_path.startswith("\\\\"):
        return str(windows_path / directory_name)
    return str(PurePosixPath(category_save_path) / directory_name)
...
class QbittorrentDownloadClient(AbstractDownloadClient):
    ...
    def __init__(self) -> None:
        ...
        if self.config.category_name in categories:
            ...
            category_mapping = cast(Mapping[str, Any], category)
            if (
                isinstance(category, Mapping)
                and category_mapping.get("savePath") == self.config.category_save_path
            ):
                log.debug(
    ...
    def download_torrent(self, indexer_result: IndexerQueryResult) -> Torrent:
        ...
        directory_name = _download_directory_name(indexer_result.title, torrent_hash)
        save_path = _download_path(self.config.category_save_path, directory_name)
        answer = None
        ...
        if not success:
            log.error(
                "Failed to download torrent, no success indicators in API answer: %s",
                answer,
            )
            msg = (
                "Failed to download torrent, no success indicators in API answer: "
                f"{answer}"
            )
