...
def get_torrent_hash(torrent: IndexerQueryResult) -> str:
    ...
    if torrent.download_url.startswith("magnet:"):
        ...
        try:
            ...
            log.debug(
                "Download URL %s uses an unsupported requests schema; following "
                "it through the configured indexer instead.",
                sanitize_url_for_logging(str(torrent.download_url)),
            )
            fallback_to_indexer = True
        except requests.exceptions.RequestException as error:
            log.error(  # noqa: TRY400 - traceback may contain a secret URL
                "Failed to download torrent file from %s (%s)",
                sanitize_url_for_logging(str(torrent.download_url)),
                type(error).__name__,
            )
            # Do not preserve the requests exception as context: its text often
            # embeds the original URL, including indexer API keys.
            msg = "Failed to download torrent file"
            raise RuntimeError(msg) from None
        except Exception:
            log.exception("Failed to download torrent file")
            raise

        # Run the fallback after leaving the InvalidSchema handler. Otherwise an
        # error in redirect resolution retains the secret-bearing exception as
        # implicit context and a caller's traceback can expose the full URL.
        if fallback_to_indexer:
        ...
        # parsing info hash
        log.debug("Parsing downloaded torrent metadata for %s", torrent.title)
        try:
