...
class MovieRepository(BaseRepository[Movie, MovieSchema]):
    ...
    async def has_movie_file(self, movie_id: MovieId) -> bool:
        """Return whether a movie is already imported or assigned a download."""
        stmt = select(MovieFile.movie_id).where(MovieFile.movie_id == movie_id).limit(1)
