...
class TvMetadataService(BaseMetadataService[Show, Show]):
    ...
    async def preview_show(
        ...
        show = await metadata_provider.get_show_metadata(external_id, language=language)
        if show is None:
            raise NotFoundError
        return show
            ...
            if fresh_season_data.external_id in existing_season_external_ids:
                ...
                season_schema = Season(
                    ...
                    monitored=(
                        ...
                        and db_show.monitor_scope == MonitorScope.ENTIRE
                    ),
                    episodes=[
                        Episode(
    ...
    async def update_all_non_ended_shows_metadata(self) -> None:
        async def get_non_ended_shows() -> list[Show]:
            return [
                show for show in await self.tv_repository.get_shows() if not show.ended
            ]
