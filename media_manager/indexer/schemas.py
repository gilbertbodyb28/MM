...
def _normalize_episode_tokens(title: str) -> str:
    """Normalize common scene separators without changing the release title."""
    normalized = re.sub(
        r"\bs(\d{1,2})[\s._-]+e(?=\d)",
        r"s\1e",
        title,
...
class IndexerQueryResult(BaseModel):
    ...
    def season(self) -> list[int]:
        title = _normalize_episode_tokens(self.title.lower())
    ...
    def episode(self) -> list[int]:
        ...
        pattern = (
            r"s\d{1,2}e(\d{1,3})"
            r"(?:\s*(?:-|\u2013|e)\s*(?:s?\d{1,2}e|e)?(\d{1,3}))?"
        )
        match = re.search(pattern, title)

        if not match:
            alternate_match = re.search(
                r"\b\d{1,2}x(\d{1,3})(?:\s*(?:-|\u2013)\s*(\d{1,3}))?\b",
                title,
            )
            if not alternate_match:
                return result
            match = alternate_match
