<script lang="ts">
	...
	import type { AutomationJob, PublicShow, RichShowTorrent, UserRead } from '$lib/api/api';
	...
	let monitorScope = $derived(show.monitor_scope);
	let monitoringLabel = $derived(
		continuousDownloadEnabled
			? monitorScope === 'specific'
				? 'Monitoring selected seasons'
				: 'Monitoring entire series, including future seasons'
			: 'Series monitoring disabled'
	);
					...
					<Card.Content class="flex flex-col items-center gap-4">
						{#if !show.ended}
							<div class="flex items-center gap-3">
								...
								/>
								<Label for="continuous-download-checkbox">{monitoringLabel}</Label>
							</div>
						{/if}
			...
			<Card.Content class="w-full overflow-x-auto">
				<Table.Root class="w-full table-fixed">
					...
					<Table.Body>
						{#if show.seasons.length > 0}
							{#each show.seasons as season (season.id)}
								...
								>
									...
									<Table.Cell class="w-[88px]"></Table.Cell>
								...
								{#if expandedSeasons.has(season.id)}
									{#each season.episodes as episode (episode.id)}
										<Table.Row class="bg-muted/20">
											...
											<Table.Cell class="w-[64px]"></Table.Cell>
										</Table.Row>
									{/each}
								{/if}
							{/each}
						{:else}
							<Table.Row>
								<Table.Cell colspan={8} class="text-center">No season data available.</Table.Cell>
							</Table.Row>
						{/if}
					</Table.Body>
				</Table.Root>
